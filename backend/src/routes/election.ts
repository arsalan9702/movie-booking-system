...
router.post('/', async (req, res) => {
  try {
    ...
    if (algorithm === 'bully') nodeManager.startBullyElection();
    else if (algorithm === 'ring') nodeManager.startRingElection();
    else return res.status(400).json({ error: 'algorithm must be bully or ring' });
    const status = await nodeManager.getClusterStatus();
