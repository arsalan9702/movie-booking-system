...
function transformPgLikePlaceholders(sql, params) {
    if (!USE_MYSQL)
        return { sql, params };
...
async function getClient() {
    if (USE_IN_MEMORY)
        throw new Error('getClient() called in in-memory mode');
    if (USE_MYSQL) {
        if (!mySqlPool)
            throw new Error('MySQL pool not initialized');
...
async function transaction(callback) {
    if (USE_IN_MEMORY)
        throw new Error('transaction() called in in-memory mode');
    if (USE_MYSQL) {
        if (!mySqlPool)
            throw new Error('MySQL pool not initialized');
...
exports.default = (pgPool || mySqlPool);
